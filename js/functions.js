...
jQuery(function () {
  ...
  const $logo = jQuery('#identity')
  const $social_links = jQuery('#social-profiles')
  const $menu = jQuery('#site-menu')
  const $content_wrap = jQuery('.content-wrap')
  const $hero_media = jQuery('.hero-media')
  const $hero_carousel = jQuery('.hero-media .owl-carousel')
  ...
  if (jQuery.fn.owlCarousel) {
    const multiple_items = jQuery('.item', $hero_carousel).length > 1
    ...
    $hero_carousel.owlCarousel({
      ...
      onInitialized: function (event) {
        ...
        jQuery('<div class="owl-expand"><a href="#"><span class="ti"></span></a></div>').insertAfter(jQuery('.owl-nav', event.target)).on('click.mccan', function (e) {
          ...
          let initialAttribs; let finalAttribs; let completed = 0
          let duration = $hero_carousel.data('expand-duration')
          const $hero_collection = $hero_media.add($hero_carousel)
          if (isNaN(duration)) {
            duration = 1000
          }
          ...
          if (!$body.hasClass('cv')) {
            ...
            $nav_buttons.animate({
              ...
              duration: duration / 2,
              complete: function () {
                const $nav = jQuery('.owl-nav', $hero_carousel)
                const $expand = jQuery('.owl-expand', $hero_carousel)
                let right_expand
                if ($body.hasClass('expanded-hero')) {
                  $nav.css({
                    right: 0
  ...
  jQuery(window).on('resize', function () {
    ...
    if ($body.hasClass('mobile-menu-opened')) {
      ...
      if ($body.hasClass('full-content')) {
        ...
        const logoHeight = jQuery('.logo', $logo).outerHeight() + $logo.offset().top * 2
